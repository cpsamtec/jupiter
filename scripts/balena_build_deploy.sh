...
if [ "$AARCH" = 'aarch64' ]; then 
    if [ "$SYSTEM_ARCH" = 'x86_64' ]; then 
        ...
        docker-compose -f docker-compose-local.yml build --build-arg NOTEBOOK_BASE_IMAGE=python:3.8-buster --build-arg 'SDS_BASE_IMAGE=balenalib/aarch64-alpine-python:3.7' samtecdeviceshare nginx notebook
    else
    ...
    if [ "$SYSTEM_ARCH" = 'x86_64' ]; then 
        docker-compose -f docker-compose-local.yml build --build-arg 'NOTEBOOK_BASE_IMAGE=python:3.8-buster' --build-arg 'SDS_BASE_IMAGE=balenalib/armv7hf-alpine-python:3.7' samtecdeviceshare nginx notebook 
    else 
