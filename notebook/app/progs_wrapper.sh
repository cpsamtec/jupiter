...
su -w "JUPI_CODESERVER_TOKEN,JUPI_AWS_ACCESS_KEY_ID,JUPI_AWS_SECRET_ACCESS_KEY,PATH,CARGO_HOME,RUSTUP_HOME,BALENA_DEVICE_UUID" - dev -c "if [ -e /code/.jupi_dev_env ]; then source /code/.jupi_dev_env; fi; PASSWORD=${JUPI_CODESERVER_TOKEN} code-server ${JUPI_CODE_SERVER_ARGS} --bind-addr 0.0.0.0:8080 /code &"
status=$?
if [ $status -ne 0 ]; then
...
JUPI_CODESERVER_TOKEN=${JUPI_CODESERVER_TOKEN:-${BALENA_DEVICE_UUID:-jupiter}}
su -w "JUPI_NOTEBOOK_TOKEN,JUPI_AWS_ACCESS_KEY_ID,JUPI_AWS_SECRET_ACCESS_KEY,PATH,JUPYTERLAB_DIR,BALENA_DEVICE_UUID,CARGO_HOME,RUSTUP_HOME" - dev  -c "cd /lab; if [ -e .jupi_dev_env ]; then source .jupi_dev_env; fi; jupyter notebook --NotebookApp.token=${JUPI_NOTEBOOK_TOKEN} ${JUPI_NOTEBOOK_ARGS} --no-browser --ip=* --port=8082 &"
